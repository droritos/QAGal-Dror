...
| AT-05 | `LevelController_HasExpansionFields` | EditMode | Validate Multi-Level variables. | `enableLevel2` is true; delay > 0. |
...
| MT-02 | Levels | Verify Multi-Level Transition. | 1. Defeat all previous waves. 2. Wait 15 seconds. | Next level waves (shielded enemies) spawn. |
| MT-03 | Platform | WebGL Build Test. | 1. Download WebGL build from GitHub. 2. Run in browser. | Game runs smoothly at 60FPS in browser. |
| MT-04 | Platform | Mobile Build Test. | 1. Open on Touch Device. 2. Drag finger on screen. | Universal touch logic moves player ship seamlessly. |
| MT-05 | Visuals | Verify Visual Effects Layering. | 1. Play Level 1. 2. Shoot enemies. | Explosions and lasers appear in front of the Nebula Background, not behind it. |
